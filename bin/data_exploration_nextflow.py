...
parser.add_argument("--dataset-name", "--name", dest="dataset_name", type=str, required=True)
parser.add_argument("--normalization-method", type=str, required=True)
parser.add_argument(
    "--normalization-stage",
    choices=["before_norm", "after_norm"],
    required=True,
    help="Whether this exploration is performed before or after normalization.",
)
parser.add_argument("--output-dir", type=str, default="Data_Exploration")
...
# Consistent species colors across all plots
species_colors = {
    "human": "blue",
    "mouse": "red"
}
...
ax.set_title("Loadings", fontsize=12)
...
if args.normalization_stage == "before_norm":
    plot_df = plot_df[plot_df["expression"] <= 25].copy()
...
# FORMAT                                                
ax.set_title("Expression distribution by tissue and species", fontsize=14)
...
# NEXTFLOW MANIFEST
# ====================================
Path("selected_normalization.txt").write_text(args.normalization_method + "\n")
...
manifest = {
    ...
    "nextflow_output_policy": "All outputs are written inside the process-local Data_Exploration folder.",
}

Path("data_exploration_manifest.json").write_text(
